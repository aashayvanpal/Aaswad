...
import MiscTable from "./order/MiscTable.js";
...
const MultiOrderShow = () => {
  ...
  useEffect(async () => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        setSelectedOrder({
          index: 0,
          date: Object.keys(orderDates[0])[0],
          orderType: "Breakfast",
        });
  ...
  const itemRender = () => {
    return (
      <>
        {orderDates[selectedOrder["index"]][selectedOrder["date"]][
      ...
      <>
        {selectedOrder !== "undefined" ? (
          <>
            ...
            {selectedOrder.index} - {selectedOrder.date} -{" "}
            {selectedOrder.orderType}
          </>
        ) : null}
      </>
  ...
  const ShowTransportTable = (medium, rate) => {
    ...
    axios
      .put(
        `/multiOrders/${id}`,
        { transport: { medium: medium, rate: rate } },
        {
          headers: {
            "x-auth": localStorage.getItem("token"),
          },
  ...
  const generateBillDelivery = () => {
    ...
    console.log("order bill check", {
      ...order,
      total: Number(rate) + Number(order.total),
      balanceAmount,
        ...
        <div id="ShowContainer1">
          <Link to="/multiorders">
            <button className="styled-btn">
              <img
                src="/static/media/back-icon.5fdd76d3.png"
                alt="backIcon"
                height="30px"
                width="30px"
              />
          ...
          <button className="styled-btn">
            <img
              src="/static/media/update-icon.67816de3.jpg"
              alt="updateIcon"
              height="30px"
              width="30px"
            />
            ...
            <div>
              ...
              {orderDates[selectedOrder["index"]][selectedOrder["date"]][
                ...
                <div>
                  ...
                  >
                    <img
                      src="/static/media/transport-icon.58cfff96.png"
                      alt="transportIcon"
                      height="30px"
                      width="30px"
                    ></img>
                  ...
                  >
                    <img
                      src="/static/media/payment-icon.2c9687e8.png"
                      alt="advanceIcon"
                      height="30px"
                      width="30px"
                    ></img>
                  ...
                  >
                    <img
                      src="/static/media/billing-icon.cfaf1bcb.jpg"
                      alt="billIcon"
                      width="30px"
                      height="30px"
                    ></img>
                  ...
                  <button
                    onClick={() => {
                      ...
                      console.log("Event Dates: ", selectedOrder.date);
                      const numberOfPeople =
                        orderDates[selectedOrder["index"]][
                          selectedOrder["date"]
                        ][selectedOrder["orderType"]].numberOfPeople;

                      downloadMultiOrderTypeBill({
                        name: fullName,
                        date: selectedOrder.date,
                        // date: eventDateNew,
                        particulars: selectedOrder.orderType,
                        numberOfPeople,
                        mobile: phoneNumber,
                        items: '',
                        transportation: rate,
                  ...
                  <button onClick={() => generateMiscItems()}>
                    <img
                      src={billIcon}
                      alt="billIcon"
                      width="30px"
